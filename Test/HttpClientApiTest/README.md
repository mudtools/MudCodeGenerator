# HttpContentType 特性测试项目

本项目用于测试和验证 Mud.CodeGenerator 的 `HttpContentType` 特性功能。
...
测试 `HttpContentType` 特性的优先级规则：

- **Body 参数的 ContentType 属性** - 最高优先级
- **方法级别的 HttpContentType 特性** - 中等优先级
- **接口级别的 HttpContentType 特性** - 低优先级
...
测试 `HttpContentType` 特性的两种使用方式和各种内容类型：

- **构造函数参数方式**：`[HttpContentType("application/json")]`
- **命名参数方式**：`[HttpContentType(ContentType="application/json")]`
...
```
HttpContentTypeTest/
├── HttpContentTypeTest.csproj          # 项目配置文件
├── IEnhancedHttpClient.cs              # 增强的HTTP客户端接口
├── Models/                            # 数据模型
│   └── TestModels.cs                  # 测试数据模型
├── HttpContentTypeTestApis/            # 测试API接口
...
生成的代码位于：
```
obj/Debug/net10.0/generated/Mud.ServiceCodeGenerator/Mud.ServiceCodeGenerator.HttpInvoke.HttpInvokeClassSourceGenerator/
```
...
- [x] 方法级别的 `HttpContentType` 特性覆盖接口级别的设置
- [x] 接口级别的 `HttpContentType` 特性被正确继承
...
// 生成的代码
request.Content = new StringContent(jsonContent, Encoding.UTF8, "text/html");
```
