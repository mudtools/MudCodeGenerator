...
public interface ISingleTestApi
{
    ...
    Task<UserDto> GetUserAsync([Path("yyyy-MM-dd")] DateTime? birthday);

    // 以下接口生成的url需要将birthday格式化处理：birthday.ToString("yyyy-MM-dd")
    [Get("/api/v1/user/{birthday}")]
    Task<UserDto> GetUser1Async([Path(FormatString = "yyyy-MM-dd")] DateTime? birthday);

    // 以下接口生成的url为：/api/v1/user?idKey=id值
    [Get("/api/v1/user")]
    Task<UserDto> GetUser2Async([Query("idKey")] string id);

    // 以下接口生成的url为：/api/v1/user?idKey=id值
    [Get("/api/v1/user")]
    Task<UserDto> GetUser3Async([Query(Name = "idKey")] string id);

    // 以下接口生成的url为：/api/v1/user?bth=birthday.ToString("yyyy-MM-dd")值
    [Get("/api/v1/user")]
    Task<UserDto> GetUser2Async([Query("bth", "yyyy-MM-dd")] DateTime? birthday);

    // 以下接口生成的url为：/api/v1/user?bth=birthday.ToString("yyyy-MM-dd")值
    [Get("/api/v1/user")]
    Task<UserDto> GetUser3Async([Query(Name = "bth", FormatString = "yyyy-MM-dd")] DateTime? birthday);
